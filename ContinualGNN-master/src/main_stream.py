...
def train(data, model, args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch in range(len(nodes) // args.batch_size):
            ...
            loss = model.loss(batch_nodes, data.positive_u_labels, data.positive_v_labels, data.negative_u_labels, data.negative_v_labels)
            loss.backward()
            model.optimizer.step()

            pos_score, neg_score = model.forward(batch_nodes, data.positive_u_labels, data.positive_v_labels, data.negative_u_labels, data.negative_v_labels)
            scores = torch.cat([pos_score, neg_score])
            labels = torch.cat([torch.ones(pos_score.shape[0]), torch.zeros(neg_score.shape[0])])
            train_scores.append(roc_auc_score(labels.detach().numpy(), scores.detach().numpy()))
            print("AUC SCORE", loss)
...
def run(args, t):
    ...
    if t > 0:
        ...
        if args.memory_size > 0:
            ...
            important_nodes_list = data.train_nodes
    ...
    avg_time, scores = train(data, ewc_model, args)
...
def evaluate(args, t):
    ...
    # Sage layers
    layers = [data.feature_size] + [args.embed_size] * (args.num_layers + 1)
...
if __name__ == "__main__":
    ...
    t_num = os.listdir(os.path.join('../data', args.data, 'stream_edges_training'))
    t_num = [int(x) for x in t_num]
    t_num.sort()
    ...
    for t in t_num:
        ...
        if args.eval == False:
            b, scores = run(args, int(t))
        else:
            b = 0
            scores = []

        #a = evaluate(args, int(t))
        #print_ans[0] += str(a[0]) + '\t'
       # print_ans[1] += str(a[1]) + '\t'
        print_ans[2] += str(b) + '\t'
       #avg_ans[0] += a[0]
    ...
    print('Scores', scores)
    print(np.round(avg_ans[0] / len(t_num), 6), np.round(avg_ans[1] / len(t_num), 6), np.round(avg_ans[2] / len(t_num), 6))
