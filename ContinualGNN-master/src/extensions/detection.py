...
def detect_simple(data, t, args):
    ...
    h_pre = get_h(model, data.cha_nodes_list + data.old_nodes_list, t - 1, args)
    h_cur = get_h(model, data.cha_nodes_list + data.old_nodes_list, t, args)

    # Calculate delta of embeddings
    delta_h = np.sum(np.abs(h_cur - h_pre), 1) 

    new_nodes_size = int(len(data.cha_nodes_list) * args.new_ratio)
    threshold = delta_h[np.argpartition(delta_h, - new_nodes_size)[- new_nodes_size]]
    new_nodes = np.array(data.cha_nodes_list + data.old_nodes_list)[np.where(delta_h > threshold)[0]]
...
def detect_bfs(data, t, args):
    ...
    h_pre = get_h(model, data.cha_nodes_list, t - 1, args)
    h_cur = get_h(model, data.cha_nodes_list, t, args)

    # Calculate delta of embeddings
    delta_h = np.sum(np.abs(h_cur - h_pre), 1)

    # Calculate influenced nodes
    nodelist, f_matrix = bfs_plus(data, data.cha_nodes_list, 2)
    ...
    new_nodes = list(set(new_nodes).intersection(set(data.cha_nodes_list + data.old_nodes_list)))
...
def get_h(model, cha_nodes_list, t, args):
    ...
    # Get embeddings
    h = model.forward(cha_nodes_list).data.cpu().numpy()
    return h
...
class AdjListsHandler(StreamDataHandler):
    def load(self, data_name, t):
        ...
        end_time = t
        for tt in range(0, len(os.listdir(os.path.join('../data', data_name, 'stream_edges_training')))):
